...
import { updateProduct } from '@/lib/actions/product-update';
import { getCategories } from '@/lib/actions/menu-categories';
import { uploadGlbAction } from '@/lib/actions/upload-glb';
import type { Product, ProductImage, MenuCategory } from '@/lib/db/schema';
...
export default function EditProductForm({ product }: EditProductFormProps) {
    ...
    // Image URL slots (up to 4) — pre-filled from existing product images
    const existingUrls = product.images?.map(img => img.imageUrl) || [];
    ...
    const handleGenerate3D = async (file: File) => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok || !data.success) throw new Error(data.error || '3D generation failed');
            setGlbUrl(data.glb_url || '');
            setIsDummyResult(data.dummy === true);
        } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (filledImages.length === 0) {
            setError('Please add at least one food image URL');
            return;
        }
        ...
        try {
            ...
            formData.append('glbUrl', glbUrl.trim());
        ...
        <div className="w-full h-full">
            ...
            <div className="mb-6 md:mb-8 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-[26px] font-normal text-black">Edit food</h1>
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm p-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-2">Click a slot to upload or replace. Slot 1 is required.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="border border-dashed border-[#4E44FD] rounded-lg p-4 mb-3 bg-[#4E44FD]/5">
                            ...
                            {generate3DError && <p className="mt-2 text-xs text-red-600">{generate3DError}</p>}
                            {glbUrl && !is3DGenerating && !isGlbUploading && (
                                <p className="mt-2 text-xs text-green-600">✓ 3D model URL set</p>
                            )}
                        </div>
                    ...
                    <div className="flex items-center gap-4 pt-4">
                        ...
                        <Link href="/home/products">
                            <button type="button" className="px-8 py-3 bg-gray-100 text-gray-700 rounded-lg font-medium text-sm hover:bg-gray-200 transition-all">
                                Cancel
                            </button>
                        </Link>
                    </div>
